...
public class BoxProcessorTest {
    ...
    public void sortCorrectlySortsWhenListIsSorted() {
        ...
        */
        ...
        // Arrange
        List<Box> sortedEvenLengthListCopy = new ArrayList<>(sortedEvenLengthList);
        List<Box> sortedOddLengthListCopy = new ArrayList<>(sortedOddLengthList);

        // Act
        boxProcessor.sort(sortedEvenLengthList);
        Collections.sort(sortedEvenLengthListCopy);
        boxProcessor.sort(sortedOddLengthList);
        Collections.sort(sortedOddLengthListCopy);

        // Assert
        assertThat(sortedEvenLengthList, equalTo(sortedEvenLengthListCopy));
        assertThat(sortedOddLengthList, equalTo(sortedOddLengthListCopy));
    ...
    public void sequentialSearchDoesNotFindBoxThatIsNotInUnsortedList() {
        ...
        List<Box> newOddLengthList = unsortedEvenLengthList;
        newOddLengthList.remove(unsortedEvenLengthList.size()-1);
        List<Box> newEvenLengthList = unsortedOddLengthList;
        newEvenLengthList.remove(unsortedOddLengthList.size()-1);


        Box boxNotInNewOddLengthList = evenLengthLastElementCopy;

        Box boxNotInNewEvenLengthList = oddLengthLastElementCopy;

        // Act
        int boxNotInNewEvenListIndex = boxProcessor.sequentialSearch(
                newEvenLengthList, boxNotInNewEvenLengthList);
        int boxNotInNewOddListIndex = boxProcessor.sequentialSearch(
                newOddLengthList, boxNotInNewOddLengthList);

        // Assert
        assertThat(boxNotInNewEvenListIndex, equalTo(BOX_NOT_FOUND_INDEX));
        assertThat(boxNotInNewOddListIndex, equalTo(BOX_NOT_FOUND_INDEX));
    }
    ...
    public void binarySearchFindsBoxWhenItisInSortedEvenLengthList() {
        ...
        // Act
        int firstBoxIndex = boxProcessor.binarySearch(sortedEvenLengthList, sortedEvenLengthFirstElementCopy);
        int middleBoxIndex = boxProcessor.binarySearch(sortedEvenLengthList, sortedEvenLengthMiddleElementCopy);
        int lastBoxIndex = boxProcessor.binarySearch(sortedEvenLengthList, sortedEvenLengthLastElementCopy);
    ...
    public void binarySearchFinsBoxWhenItisInSortedOddLengthList() {
        // Act
        int firstBoxIndex = boxProcessor.binarySearch(sortedOddLengthList, sortedOddLengthFirstElementCopy);
        int middleBoxIndex = boxProcessor.binarySearch(sortedOddLengthList, sortedOddLengthMiddleElementCopy);
        int lastBoxIndex = boxProcessor.binarySearch(sortedOddLengthList, sortedOddLengthLastElementCopy);
    ...
    public void binarySearchDoesNotFindBoxThatIsNotInSortedList() {
        ...
        // Arrange
        List<Box> newOddLengthList = sortedEvenLengthList;
        newOddLengthList.remove(sortedEvenLengthList.size()-1);
        List<Box> newEvenLengthList = sortedOddLengthList;
        newEvenLengthList.remove(sortedOddLengthList.size()-1);


        Box boxNotInNewOddLengthList = sortedEvenLengthLastElementCopy;

        Box boxNotInNewEvenLengthList = sortedOddLengthLastElementCopy;


        // Act
        int boxNotInNewEvenListIndex = boxProcessor.binarySearch(
                newEvenLengthList, boxNotInNewEvenLengthList);
        int boxNotInNewOddListIndex = boxProcessor.binarySearch(
                newOddLengthList, boxNotInNewOddLengthList);
